 ...
 *                biHeight value were modified to accomodate both inverted (normal) and top down images. The image is stored in the normal manner
 *                regardless of how it was read in.
 *
 * Rev: 11/20/14  The functionality contained in the main method was modularized. Initial tests suggest that the functionality has been preserved.  
 *                A method called process has been added, among other things, that should be called in the main method for whatever is defined to be
 *                full functionality.  At this point, that's just reading a bitmap and then outputting that information to another file. 
 * 
 * Classes in the file:
 *  RgbQuad
 *  DibDump
...
{
   static final int CROPPED_IMAGE_ROW = 45;
   static final int CROPPED_IMAGE_COL = 45;
...
// The true color pels
   int[][] imageArray;
   int[] com = new int[2];
   int[] compressedImage = new int[CROPPED_IMAGE_ROW* CROPPED_IMAGE_COL];
 ...
 */
   public int colorToGrayscale(int pel)
   {
      ...
      int lum = (int) Math.round(0.3 * (double) rgb.red + 0.589
            * (double) rgb.green + 0.11 * (double) rgb.blue);

      return rgbToPel(lum, lum, lum);
   }
   
   /**
    * Takes a 2D array and compresses it into a 1D array.  This is done by "snaking"
    * through the 2D array and copying each value into the 1D array.
    */
   public void compressArray()
   {
      ...
      for (int r = 0; r < imageArray.length; r++)
      {
         if (r % 2 == 0)
         {
            ...
            {
               compressedImage[u] = imageArray[r][c];
               u++;
            }
         }
         else
         {
            for (int c = imageArray[r].length - 1; c >= 0; c--)
            {
               compressedImage[u] = imageArray[r][c];
               u++;
            }
   ...
   /**
    * Loops through the imageArray variable, taking each value and
    * 1) converting it to a grayscale number (see colorToGrayscale)
    * 2) inverting the integer while masking the red and green values. Because we are 
    * now working in grayscale, the rgb values are all the same, so keeping only one
    * preserves the information while cutting down on size. The inversion is used to 
    * reduce the number of whiter (higher-valued) pixels and increase the number of 
    * blacker (lower-valued) pixels.  The catch is that the value stored in the image 
    * array is actually a shade of blue; another method is needed (post-processing) to
    * reconvert to grayscale.
    */
   public void invertAndConvertToGrayscale()
   {
      for (int r = 0; r < imageArray.length; r++)
      {
         for (int c = 0; c < imageArray[r].length; c++)
         {
            imageArray[r][c] = invertPel(colorToGrayscale(imageArray[r][c]));
         }
      }
      return;
   }
   
   public int invertPel(int pel)
   {
      return ~pel & 0x000000FF;
   }
   
   public void colorCOM()
   {
      int r = com[1];
      int c = com[0];
      imageArray[r][c] = rgbToPel(0,imageArray[r][c],0);
   }
   
   /**
    * Loops through the imageArray variable and calculates the center of mass of 
    * the picture by summing the product of each pixel's location (determined by its array parameters)
    * and its mass (determined by the value stored at that location). The x and y coordinates
    * are calculated separately and are returned in an array, with the first number representing
   ...
   public void calculateCenterOfMass()
   {
      ...
      for (int r = 0; r < imageArray.length; r++)
      {
         for (int c = 0; c < imageArray[r].length; c++)
         {
            com[0] += c * imageArray[r][c];
            com[1] += r * imageArray[r][c];
            totalMass += imageArray[r][c];
         }
      }
   ...
   /**
    * Crops the image down to a certain size (specified by the CROPPED_IMAGE_SIZE parameter).
    * The center of the new image will be the center of mass of the original.
    * 
    * A double for loop is used to iterate through the original image (imageArray),
    * using CROPPED_IMAGE_SIZE as a means of determining the starting and ending rows and columns.
    * 
    * Precondition: com is an array of size two, with the first element representing the
    * x coordinate of the center of mass, and the second element representing the y 
    * coordinate of the center of mass.
    * @param com the array containing the center of mass information
    * @param CROPPED_IMAGE_SIZE the desired length of one side of the image
    */
   public void cropImage()
   {
      int[][] croppedImage = new int[CROPPED_IMAGE_ROW][CROPPED_IMAGE_COL];
      int u = 0, v = 0;
      for (int r = com[1] - (CROPPED_IMAGE_ROW)/2; r < com[1] + (CROPPED_IMAGE_ROW)/2; r++)
      {
         for (int c = com[0] - (CROPPED_IMAGE_COL)/2; c < com[0] + (CROPPED_IMAGE_COL)/2; c++)
         {
            croppedImage[u][v] = imageArray[r][c];
            v++;
         }
         u++;
         v = 0;
      ...
      bmpInfoHeader_biHeight = CROPPED_IMAGE_ROW;
      bmpInfoHeader_biWidth = CROPPED_IMAGE_COL;
 ...
 */
   public static void main(String[] args)
   {
      ...
      DibDump dibdumper = new DibDump(); // needed to get to the byte swapping
                                         // methods
      
      /*if (args.length > 0)
         dibdumper.inFileName = args[0];
      else
         dibdumper.inFileName = "test4.bmp";

      if (args.length > 1)
         dibdumper.outFileName = args[1];
      else
         dibdumper.outFileName = "test3.bmp";
      ...
      for (int i = 0; i < 26; i++)
      {
         dibdumper.inFileName = "small_letters/" + letters[i] + ".bmp";
         dibdumper.outFileName = "small_cropped/" + letters[i] + ".bmp";
         dibdumper.process();
      }*/
      
      
      dibdumper.inFileName = "original_faces/ein.bmp";
      dibdumper.outFileName = "test_faces/ein.bmp";
      dibdumper.process();
      dibdumper.inFileName = "original_faces/nixon.bmp";
      dibdumper.outFileName = "test_faces/nixon.bmp";
      dibdumper.process();
   ...
   *
   * @param in the input object used to access the bitmap
   * 
   * @throws an IOException error if the DataInputStream is incorrectly initialized
   *
   */
   public void initializeFileHeaderInfo(DataInputStream in) throws IOException
   {
  ...
  * @throws an IOException error if the DataInputStream is incorrectly initialized
  */
   public void initializeInfoHeaderInfo(DataInputStream in) throws IOException
   {
...
// stored on disc upsidedown (which means it is actually rightside up).
   ...
   */   
   public void determineNumColors()
   {
   ...
   *
   */
   public void initializeColorPallet(DataInputStream in) throws IOException
   {

      if (bmpInfoHeader_biClrUsed > 0)
         numberOfColors = bmpInfoHeader_biClrUsed;

      for (i = 0; i < numberOfColors; ++i) // Read in the color table (or not if
                                           // numberOfColors is zero)
      {
         ...
         // values to be int, we can shift and then OR the values
         // to build up the color. Since we are reading one byte at a time,
         // there are no "endian" issues.

         colorPallet[i] = (rgbQuad_rgbRed << 16) | (rgbQuad_rgbGreen << 8)
               | rgbQuad_rgbBlue;
   ...
   * @throws an IOException error if the DataInputStream is incorrectly initialized
   */
   public void BitmapInputCase1(DataInputStream in) throws IOException
   {
      ...
      for (int row = 0; row < bmpInfoHeader_biHeight; ++row) // read over the
                                                             // rows
      {
         if (topDownDIB)
            i = row;
         ...
         for (j = 0; j < iDeadBytes; ++j)
            in.readUnsignedByte(); // Now read in the "dead bytes" to pad to a 4
                                   // byte boundary
      }
      return;
   }
   ...
   */
   ...
   public void BitmapInputCase2(DataInputStream in) throws IOException
   {
   ...
   */
   ...
   public void BitmapInputCase4(DataInputStream in) throws IOException
   {
                                                             ...
                                                             // rows
      {
         ...
         for (j = 0; j < iBytesPerRow; ++j)
         {
            ...
            {
               iColumn = j * 2 + k; // 1 - k is needed to have High, Low nibble
                                    // ordering for the image.
               pel = colorPallet[(iByteVal >> ((1 - k) * 4)) & 0x0F]; // shift 4 bits at a time
               imageArray[i][iColumn] = pel;
            }
         }

         if (iTrailingBits > 0) // pick up the trailing nibble for images that
                                // are not mod 2 columns wide
         {
            iByteVal = in.readUnsignedByte();

            iColumn = iBytesPerRow * 2;
            pel = colorPallet[(iByteVal >> 4) & 0x0F]; // The High nibble is the
                                                       // last remaining pel
            imageArray[i][iColumn] = pel;
         }
         for (j = 0; j < iDeadBytes; ++j)
   ...
   * @throws an IOException error if the DataInputStream is incorrectly initialized
   */
   public void BitmapInputCase8(DataInputStream in) throws IOException
   {
   ...
   * @throws an IOException error if the DataInputStream is incorrectly initialized
   */
   public void BitmapInputCase16(DataInputStream in) throws IOException
   {
   ...
   * @throws an IOException error if the DataInputStream is incorrectly initialized
   */
   public void BitmapInputCase24(DataInputStream in) throws IOException
   {
      iPelsPerRow = bmpInfoHeader_biWidth;
      ...
      for (int row = 0; row < bmpInfoHeader_biHeight; ++row) // read over the rows
      {
         ...
         for (j = 0; j < iPelsPerRow; ++j) // j is now just the column counter
         {
            ...
            pel = (rgbQuad_rgbRed << 16) | (rgbQuad_rgbGreen << 8)
                  | rgbQuad_rgbBlue;
            imageArray[i][j] = pel;
         }
         for (j = 0; j < iDeadBytes; ++j)
   ...
   * @throws an IOException error if the DataInputStream is incorrectly initialized
   */
   public void BitmapInputCase32(DataInputStream in) throws IOException
   {
      iPelsPerRow = bmpInfoHeader_biWidth;
      for (int row = 0; row < bmpInfoHeader_biHeight; ++row) // read over the rows
      {
   ...
   * @throws an IOException error if the DataInputStream is incorrectly initialized
   */
   public void readInBitmap(DataInputStream in) throws IOException
   {
      ...
      switch (bmpInfoHeader_biBitCount)
      {
         ...
         case 8: // 1 byte, 1 pel, Works
            BitmapInputCase8(in);
            break;
         case 16: // Should work, not tested.
            BitmapInputCase16(in);
            break;
         case 24: // Works
            BitmapInputCase24(in);
            break;
         case 32: // Works
            BitmapInputCase32(in);
            break;
         default: // Oops
   ...
   //Covers the steps needed to input the information from a bitmap.  See individual
   //methods for more detailed comments.
   public void importBitmapInfo()
   {
      try // lots of things can go wrong when doing file i/o
      {
         ...
         // Read and Convert to big endian
         initializeFileHeaderInfo(in);
         initializeInfoHeaderInfo(in);
   
         fixImageHeight();
   
         determineNumColors();
         
         initializeColorPallet(in);
   ...
   /*
    * Outputs the header information to the file via the specified output stream.
    * 
    * @param out the output object used to write to the bitmap
    * 
    * @throws IOException if the DataOutputStream is incorrectly initialized
    */
   public void outputHeaderInfo(DataOutputStream out) throws IOException
   {
...
// BITMAPINFOHEADER
   ...
   /*
    * Inputs the bitmap from a file and writes that info to another file.
    */
   public void process()
   {
      importBitmapInfo();
      invertAndConvertToGrayscale();
